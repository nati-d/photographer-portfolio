            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.label}
                  href={link.href}
                  className={`px-6 py-2 text-xs md:text-sm font-semibold uppercase tracking-widest focus:outline-none border-l border-border first:border-l-0 bg-transparent shadow-none transition-colors hover:text-accent ${isActive ? "text-accent font-bold" : ""}`}
        ...
        {mobileOpen && (
          ...
          >
            <div className="flex items-center justify-between px-6 py-4 border-b border-border">
              <span className="text-xl font-extrabold tracking-widest text-accent" style={{ fontFamily: 'var(--font-heading)' }}>
                DAMIEN
              </span>
              <button
              ...
              {navLinks.map((link) => {
                ...
                return (
                  <Link
                    key={link.label}
                    href={link.href}
                    className={`block w-full px-4 py-3 rounded-md text-lg font-semibold uppercase tracking-widest text-foreground hover:bg-primary/80 hover:text-accent transition-colors border border-border mb-2 ${isActive ? "text-accent font-bold" : ""}`}
                    style={{ fontFamily: 'var(--font-heading)' }}
                    onClick={() => setMobileOpen(false)}
                  >
                    {link.label}
